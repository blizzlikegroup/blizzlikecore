...
Unit* ScriptedAI::DoSelectLowestHpFriendly(float fRange, uint32 uiMinHPDiff)
{
    ...
    BlizzLike::MostHPMissingInRangeCheck u_check(m_creature, fRange, uiMinHPDiff);
    BlizzLike::UnitLastSearcher<BlizzLike::MostHPMissingInRangeCheck> searcher(pUnit, u_check);
...
std::list<Creature*> ScriptedAI::DoFindFriendlyCC(float fRange)
{
    ...
    BlizzLike::FriendlyCCedInRangeCheck u_check(m_creature, fRange);
    BlizzLike::CreatureListSearcher<BlizzLike::FriendlyCCedInRangeCheck> searcher(pList, u_check);
...
std::list<Creature*> ScriptedAI::DoFindFriendlyMissingBuff(float fRange, uint32 uiSpellId)
{
    ...
    BlizzLike::FriendlyMissingBuffInRangeCheck u_check(m_creature, fRange, uiSpellId);
    BlizzLike::CreatureListSearcher<BlizzLike::FriendlyMissingBuffInRangeCheck> searcher(pList, u_check);
...
Player* ScriptedAI::GetPlayerAtMinimumRange(float fMinimumRange)
{
    ...
    BlizzLike::AnyPlayerInObjectRangeCheck check(m_creature, fMinimumRange);
    BlizzLike::PlayerSearcher<BlizzLike::AnyPlayerInObjectRangeCheck> searcher(pPlayer, check);
...
// It is assumed the information is found elswehere and can be handled by BlizzLike. So far no luck finding such information/way to extract it.
enum
{
    NPC_BROODLORD               = 12017,
