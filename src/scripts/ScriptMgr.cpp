...
void LoadDatabase()
{
    std::string strSD2DBinfo = SD2Config.GetStringDefault("ScriptsDatabaseInfo", "");
...
void InitScriptLibrary()
{
    ...
    std::string sd2LogFile = SD2Config.GetStringDefault("ScriptsErrorLogFile", "ScriptsErrors.log");
...
void DoOrSimulateScriptTextForMap(int32 iTextEntry, uint32 uiCreatureEntry, Map* pMap, Creature* pCreatureSource /*=NULL*/, Unit* pTarget /*=NULL*/)
{
    ...
    BlizzLikeStringLocale const* pData = GetBlizzLikeStringData(iTextEntry);
    if (!pData)
    {
