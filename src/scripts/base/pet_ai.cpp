...
{
    if (!m_creature->isAlive())                             // should not be needed, isAlive is checked in BlizzLike before calling UpdateAI
        return;

    // UpdateAllies() is done in the generic PetAI in BlizzLike, but we can't do this from script side.
    // Unclear what side effects this has, but is something to be resolved from BlizzLike.
    ...
    if (m_creature->getVictim())                            // in combat
    {
        if (!m_creature->getVictim()->isTargetableForAttack())
        {
            ...
            // doesn't normally reach this, because of how petAi is designed in BlizzLike. CombatStop
