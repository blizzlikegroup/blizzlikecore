...
struct BLIZZLIKE_DLL_DECL npc_giltharesAI : public npc_escortAI
{
...
struct BLIZZLIKE_DLL_DECL npc_taskmaster_fizzuleAI : public ScriptedAI
{
...
struct BLIZZLIKE_DLL_DECL npc_twiggy_flatheadAI : public ScriptedAI
{
    ...
    if (pPlayer->isAlive() && !pPlayer->isGameMaster() && pPlayer->GetQuestStatus(QUEST_AFFRAY) == QUEST_STATUS_INCOMPLETE)
    {
        ...
        if (npc_twiggy_flatheadAI* pTwiggyAI = dynamic_cast<npc_twiggy_flatheadAI*>(pCreature->AI()))
        {
            if (pTwiggyAI->CanStartEvent(pPlayer))
                return false;                               // ok to let BlizzLike process further
        }
...
struct BLIZZLIKE_DLL_DECL npc_wizzlecranks_shredderAI : public npc_escortAI
{
