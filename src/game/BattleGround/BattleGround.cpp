...
namespace BlizzLike
{
    ...
    class BattleGround2ChatBuilder
    {
        public:
            ...
            {
                char const* text = sObjectMgr.GetBlizzLikeString(i_textId, loc_idx);
                char const* arg1str = i_arg1 ? sObjectMgr.GetBlizzLikeString(i_arg1, loc_idx) : "";
                char const* arg2str = i_arg2 ? sObjectMgr.GetBlizzLikeString(i_arg2, loc_idx) : "";
...
uint32 BattleGround::GetBonusHonorFromKill(uint32 kills) const
{
    ...
    return (uint32)BlizzLike::Honor::hk_honor_at_level(GetMaxLevel(), kills);
}
...
void BattleGround::SendYellToAll(int32 entry, uint32 language, ObjectGuid guid)
{
    ...
    if (!source)
        return;
    BlizzLike::BattleGroundYellBuilder bg_builder(language, entry, source);
    BlizzLike::LocalizedPacketDo<BlizzLike::BattleGroundYellBuilder> bg_do(bg_builder);
    BroadcastWorker(bg_do);
}
...
void BattleGround::SendYell2ToAll(int32 entry, uint32 language, ObjectGuid guid, int32 arg1, int32 arg2)
{
    ...
    if (!source)
        return;
    BlizzLike::BattleGround2YellBuilder bg_builder(language, entry, source, arg1, arg2);
    BlizzLike::LocalizedPacketDo<BlizzLike::BattleGround2YellBuilder> bg_do(bg_builder);
    BroadcastWorker(bg_do);
}
