...
void BattleGroundAV::HandleQuestComplete(uint32 questid, Player* player)
{
    ...
    BLIZZLIKE_ASSERT(teamIdx != BG_AV_TEAM_NEUTRAL);
...
void BattleGroundAV::UpdateScore(BattleGroundTeamIndex teamIdx, int32 points)
{
    ...
    BLIZZLIKE_ASSERT(teamIdx == BG_TEAM_ALLIANCE || teamIdx == BG_TEAM_HORDE);
...
void BattleGroundAV::EventPlayerDestroyedPoint(BG_AV_Nodes node)
{
    ...
    BLIZZLIKE_ASSERT(m_Nodes[node].Owner != BG_AV_TEAM_NEUTRAL)
...
void BattleGroundAV::ChangeMineOwner(uint8 mine, BattleGroundAVTeamIndex teamIdx)
{
    ...
    BLIZZLIKE_ASSERT(mine == BG_AV_NORTH_MINE || mine == BG_AV_SOUTH_MINE);
...
{
    BLIZZLIKE_ASSERT(GetStatus() == STATUS_IN_PROGRESS);
    ...
    if (m_Nodes[node].TotalOwner == BG_AV_TEAM_NEUTRAL)     // initial snowfall capture
    {
        ...
        BLIZZLIKE_ASSERT(node == BG_AV_NODES_SNOWFALL_GRAVE);  // currently the only neutral grave
...
{
    BLIZZLIKE_ASSERT(m_Nodes[node].TotalOwner != BattleGroundAVTeamIndex(teamIdx));
    BLIZZLIKE_ASSERT(m_Nodes[node].Owner != BattleGroundAVTeamIndex(teamIdx));
    // only assault an assaulted node if no totalowner exists:
    BLIZZLIKE_ASSERT(m_Nodes[node].State != POINT_ASSAULTED || m_Nodes[node].TotalOwner == BG_AV_TEAM_NEUTRAL);
...
{
    BLIZZLIKE_ASSERT(m_Nodes[node].State == POINT_ASSAULTED);
...
void BattleGroundAV::DefendNode(BG_AV_Nodes node, BattleGroundTeamIndex teamIdx)
{
    BLIZZLIKE_ASSERT(m_Nodes[node].TotalOwner == BattleGroundAVTeamIndex(teamIdx));
    BLIZZLIKE_ASSERT(m_Nodes[node].Owner != BattleGroundAVTeamIndex(teamIdx));
    BLIZZLIKE_ASSERT(m_Nodes[node].State != POINT_CONTROLLED);
    m_Nodes[node].PrevOwner  = m_Nodes[node].Owner;
    m_Nodes[node].Owner      = BattleGroundAVTeamIndex(teamIdx);
    m_Nodes[node].PrevState  = m_Nodes[node].State;
    m_Nodes[node].State      = POINT_CONTROLLED;
