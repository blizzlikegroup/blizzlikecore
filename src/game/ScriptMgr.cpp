    ...
    do
    {
        ...
        switch (tmp.command)
        {
            case SCRIPT_COMMAND_TALK:                       // 0
            {
                ...
                // if (!GetBlizzLikeStringLocale(tmp.dataint)) will be checked after db_script_string loading
                break;
            }
            ...
            case SCRIPT_COMMAND_TELEPORT_TO:                // 6
            {
                ...
                if (!BlizzLike::IsValidMapCoord(tmp.x, tmp.y, tmp.z, tmp.o))
                {
...
{
    sObjectMgr.LoadBlizzLikeStrings(WorldDatabase, "db_script_string", MIN_DB_SCRIPT_STRING_ID, MAX_DB_SCRIPT_STRING_ID, true);

    std::set<int32> ids;

    for (int32 i = MIN_DB_SCRIPT_STRING_ID; i < MAX_DB_SCRIPT_STRING_ID; ++i)
        if (sObjectMgr.GetBlizzLikeStringLocale(i))
            ids.insert(i);
    ...
    for (ScriptMapMap::const_iterator itrMM = scripts.second.begin(); itrMM != scripts.second.end(); ++itrMM)
    {
        for (ScriptMap::const_iterator itrM = itrMM->second.begin(); itrM != itrMM->second.end(); ++itrM)
        {
            if (itrM->second.command == SCRIPT_COMMAND_TALK)
            {
                for (int i = 0; i < MAX_TEXT_ID; ++i)
                {
                    if (itrM->second.textId[i] && !sObjectMgr.GetBlizzLikeStringLocale(itrM->second.textId[i]))
    ...
    if (m_script->buddyEntry)
    {
        ...
        else                                                // Buddy by entry
        {
            ...
            if (m_script->IsCreatureBuddy())
            {
                ...
                BlizzLike::NearestCreatureEntryWithLiveStateInObjectRangeCheck u_check(*pSearcher, m_script->buddyEntry, true, false, m_script->searchRadiusOrGuid, true);
                BlizzLike::CreatureLastSearcher<BlizzLike::NearestCreatureEntryWithLiveStateInObjectRangeCheck> searcher(pCreatureBuddy, u_check);
            ...
            else
            {
                ...
                BlizzLike::NearestGameObjectEntryInObjectRangeCheck u_check(*pSearcher, m_script->buddyEntry, m_script->searchRadiusOrGuid);
                BlizzLike::GameObjectLastSearcher<BlizzLike::NearestGameObjectEntryInObjectRangeCheck> searcher(pGOBuddy, u_check);
    ...
    switch (m_script->command)
    {
        ...
        case SCRIPT_COMMAND_TERMINATE_SCRIPT:               // 31
        {
            ...
            if (m_script->terminateScript.npcEntry)
            {
                ...
                Creature* pCreatureBuddy = NULL;
                BlizzLike::NearestCreatureEntryWithLiveStateInObjectRangeCheck u_check(*pSearcher, m_script->terminateScript.npcEntry, true, false, m_script->terminateScript.searchDist, true);
                BlizzLike::CreatureLastSearcher<BlizzLike::NearestCreatureEntryWithLiveStateInObjectRangeCheck> searcher(pCreatureBuddy, u_check);
...
ScriptLoadResult ScriptMgr::LoadScriptLibrary(const char* libName)
{
    ...
    name = BLIZZLIKE_SCRIPT_PREFIX + name + BLIZZLIKE_SCRIPT_SUFFIX;

    m_hScriptLib = BLIZZLIKE_LOAD_LIBRARY(name.c_str());
...
#   define GET_SCRIPT_HOOK_PTR(P,N)             \
    ...
    // let check used worldserver revision for build library (unsafe use with different revision because changes in inline functions, define and etc)
    char const*(BLIZZLIKE_IMPORT * pGetBlizzLikeRevStr)();

    GET_SCRIPT_HOOK_PTR(pGetBlizzLikeRevStr,              "GetBlizzLikeRevStr");
...
#   undef GET_SCRIPT_HOOK_PTR
    ...
    if (strcmp(pGetBlizzLikeRevStr(), REVISION_NR) != 0)
    {
...
void ScriptMgr::UnloadScriptLibrary()
{
    ...
    BLIZZLIKE_CLOSE_LIBRARY(m_hScriptLib);
    m_hScriptLib = NULL;
