...
class Spell
{
        friend struct BlizzLike::SpellNotifierPlayer;
    ...
    public:
        ...
        static void BLIZZLIKE_DLL_SPEC SendCastResult(Player* caster, SpellEntry const* spellInfo, uint8 cast_count, SpellCastResult result, bool isPetCastResult = false);
...
namespace BlizzLike
{
    struct BLIZZLIKE_DLL_DECL SpellNotifierPlayer              // Currently unused. When put to use this one requires handling for source-location (smilar to below)
    {
    ...
    struct BLIZZLIKE_DLL_DECL SpellNotifierCreatureAndPlayer
    {
        Spell::UnitList* i_data;
        Spell& i_spell;
