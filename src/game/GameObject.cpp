        ...
        case GO_READY:
        {
            ...
            if (isSpawned())
            {
                ...
                if (goInfo->type == GAMEOBJECT_TYPE_TRAP)   // traps
                {
                    ...
                    BlizzLike::UnitSearcher<BlizzLike::AnyUnfriendlyUnitInObjectRangeCheck> checker(enemy, u_check);
...
void GameObject::TriggerLinkedGameObject(Unit* target)
{
...

    {
        ...
        BlizzLike::NearestGameObjectEntryInObjectRangeCheck go_check(*this, trapEntry, range);
...
GameObject* GameObject::LookupFishingHoleAround(float range)
{
    ...
    BlizzLike::NearestGameObjectFishingHoleCheck u_check(*this, range);
    BlizzLike::GameObjectSearcher<BlizzLike::NearestGameObjectFishingHoleCheck> checker(ok, u_check);
    Cell::VisitGridObjects(this, checker, range);
...
void GameObject::Use(Unit* user)
{
    ...
    BLIZZLIKE_ASSERT(user || PrintEntryError("GameObject::Use (without user)"));
...
void GameObject::TickCapturePoint()
{
    ...
    BlizzLike::AnyPlayerInCapturePointRange u_check(this, radius);
    BlizzLike::PlayerListSearcher<BlizzLike::AnyPlayerInCapturePointRange> checker(capturingPlayers, u_check);
