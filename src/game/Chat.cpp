...
{
    return value ?  GetBlizzLikeString(LANG_ON) : GetBlizzLikeString(LANG_OFF);
}
...
{
    return sObjectMgr.GetBlizzLikeStringForDBCLocale(entry);
}
...
std::string ChatHandler::PrepareStringNpcOrGoSpawnInformation(uint32 guid)
{
    ...
    if (uint16 pool_id = sPoolMgr.IsPartOfAPool<T>(guid))
    {
        ...
        {
            char buffer[100];
            const char* format = GetBlizzLikeString(LANG_NPC_GO_INFO_POOL_EVENT_STRING);
        ...
        {
            char buffer[100];
            const char* format = GetBlizzLikeString(LANG_NPC_GO_INFO_POOL_STRING);
    ...
    {
        char buffer[100];
        const char* format = GetBlizzLikeString(LANG_NPC_GO_INFO_EVENT_STRING);
