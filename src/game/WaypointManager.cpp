...
void WaypointManager::Load()
{
    ...
    else
    {
        ...
        do
        {
            ...
            if (!BlizzLike::IsValidMapCoord(node.x, node.y, node.z, node.orientation))
            {
                ...
                BlizzLike::NormalizeMapCoord(node.x);
                BlizzLike::NormalizeMapCoord(node.y);
...
inline void CheckWPText(bool isTemplate, uint32 entryOrGuid, uint32 point, WaypointBehavior* be, std::set<int32>& ids)
{
    ...
    for (int j = 0; j < MAX_WAYPOINT_TEXT; ++j)
    {
        ...
        if (!sObjectMgr.GetBlizzLikeStringLocale(be->textid[j]))
        {
