...
void Map::MessageBroadcast(WorldObject const* obj, WorldPacket* msg)
{
    ...
    BlizzLike::ObjectMessageDeliverer post_man(msg);
...
void Map::Update(const uint32& t_diff)
{
    ...
    if (!IsBattleGroundOrArena())
    {
        for (GridRefManager<NGridType>::iterator i = GridRefManager<NGridType>::begin(); i != GridRefManager<NGridType>::end();)
        {
            ...
            BLIZZLIKE_ASSERT(grid->GetGridState() >= 0 && grid->GetGridState() < MAX_GRID_STATE);
...
{
    BLIZZLIKE_ASSERT(CheckGridIntegrity(creature, false));

    Cell old_cell = creature->GetCurrentCell();
    Cell new_cell(BlizzLike::ComputeCellPair(x, y));
    ...
    BLIZZLIKE_ASSERT(CheckGridIntegrity(creature, true));
}
...
{
    BLIZZLIKE_ASSERT(obj->GetMapId() == GetId() && obj->GetInstanceId() == GetInstanceId());
...
{
    BLIZZLIKE_ASSERT(x < MAX_NUMBER_OF_GRIDS);
    BLIZZLIKE_ASSERT(y < MAX_NUMBER_OF_GRIDS);
...
void Map::AddToActive(WorldObject* obj)
{
    ...
    Cell cell = Cell(BlizzLike::ComputeCellPair(obj->GetPositionX(), obj->GetPositionY()));
    EnsureGridLoaded(cell);
...
{
    BLIZZLIKE_ASSERT(i_mapEntry->IsDungeon());
    ...
    else
    {
        ...
        else
        {
            ...
            if (!playerBind)
                ...
                // cannot jump to a different instance without resetting it
                BLIZZLIKE_ASSERT(playerBind->state == GetPersistentState());
        }
    }
...
void Map::MonsterYellToMap(CreatureInfo const* cinfo, int32 textId, uint32 language, Unit const* target, uint32 senderLowGuid /*= 0*/) const
{
    ...
    BlizzLike::LocalizedPacketDo<StaticMonsterChatBuilder> say_do(say_build);
