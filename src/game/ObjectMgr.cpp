...
void ObjectMgr::LoadPointsOfInterest()
{
    ...
    do
    {
        ...
        if (!BlizzLike::IsValidMapCoord(POI.x, POI.y))
        {
...
static char* SERVER_SIDE_SPELL      = "BlizzLike server-side spell";
...
inline void _DoStringError(int32 entry, char const* text, ...)
{
    ...
    else if (entry < MIN_DB_SCRIPT_STRING_ID)               // BlizzLike string error
        sLog.outError("%s", buf);
    else // if (entry > MIN_DB_SCRIPT_STRING_ID)            // DB script text error
        sLog.outErrorDb("DB-SCRIPTS: %s", buf);
}

bool ObjectMgr::LoadBlizzLikeStrings(DatabaseType& db, char const* table, int32 min_value, int32 max_value, bool extra_content)
{
...
bool LoadBlizzLikeStrings(DatabaseType& db, char const* table, int32 start_value, int32 end_value, bool extra_content)
{
    // MAX_DB_SCRIPT_STRING_ID is max allowed negative value for scripts (scrpts can use only more deep negative values
    // start/end reversed for negative values
    if (start_value > MAX_DB_SCRIPT_STRING_ID || end_value >= start_value)
    {
        sLog.outErrorDb("Table '%s' attempt loaded with reserved by BlizzLike range (%d - %d), strings not loaded.", table, start_value, end_value + 1);
        return false;
    }

    return sObjectMgr.LoadBlizzLikeStrings(db, table, start_value, end_value, extra_content);
}
...
BlizzLikeStringLocale const* GetBlizzLikeStringData(int32 entry)
{
    return sObjectMgr.GetBlizzLikeStringLocale(entry);
}
...
{
    BlizzLikeStringLocale const* data = sObjectMgr.GetBlizzLikeStringLocale(entry);
