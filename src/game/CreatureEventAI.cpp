...
Unit* CreatureEventAI::DoSelectLowestHpFriendly(float range, uint32 MinHPDiff)
{
    ...
    BlizzLike::MostHPMissingInRangeCheck u_check(m_creature, range, MinHPDiff);
...
{
    BlizzLike::FriendlyCCedInRangeCheck u_check(m_creature, range);
    BlizzLike::CreatureListSearcher<BlizzLike::FriendlyCCedInRangeCheck> searcher(_list, u_check);
    Cell::VisitGridObjects(m_creature, searcher, range);
}

void CreatureEventAI::DoFindFriendlyMissingBuff(std::list<Creature*>& _list, float range, uint32 spellid)
{
    BlizzLike::FriendlyMissingBuffInRangeCheck u_check(m_creature, range, spellid);
    BlizzLike::CreatureListSearcher<BlizzLike::FriendlyMissingBuffInRangeCheck> searcher(_list, u_check);
    Cell::VisitGridObjects(m_creature, searcher, range);
}
