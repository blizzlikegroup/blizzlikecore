...
bool ChatHandler::HandleGPSCommand(char* args)
{
    ...
    DEBUG_LOG("Player %s GPS call for %s '%s' (%s: %u):",
              m_session ? GetNameLink().c_str() : GetBlizzLikeString(LANG_CONSOLE_COMMAND),
              (obj->GetTypeId() == TYPEID_PLAYER ? "player" : "creature"), obj->GetName(),
              (obj->GetTypeId() == TYPEID_PLAYER ? "GUID" : "Entry"), (obj->GetTypeId() == TYPEID_PLAYER ? obj->GetGUIDLow() : obj->GetEntry()));

    DEBUG_LOG(GetBlizzLikeString(LANG_MAP_POSITION),
...
bool ChatHandler::HandleNamegoCommand(char* args)
{
    ...
    else
    {
        ...
        PSendSysMessage(LANG_SUMMONING, nameLink.c_str(), GetBlizzLikeString(LANG_OFFLINE));
...
bool ChatHandler::HandleModifyEnergyCommand(char* args)
{
    ...
    DETAIL_LOG(GetBlizzLikeString(LANG_CURRENT_ENERGY), chr->GetMaxPower(POWER_ENERGY));
...
bool ChatHandler::HandleModifyMoneyCommand(char* args)
{
    ...
    if (addmoney < 0)
    {
        ...
        DETAIL_LOG(GetBlizzLikeString(LANG_CURRENT_MONEY), moneyuser, addmoney, newmoney);
        if (newmoney <= 0)
        {
    ...
    DETAIL_LOG(GetBlizzLikeString(LANG_NEW_MONEY), moneyuser, addmoney, chr->GetMoney());
...
bool ChatHandler::HandleTeleNameCommand(char* args)
{
    ...
    else
    {
        ...
        PSendSysMessage(LANG_TELEPORTING_TO, nameLink.c_str(), GetBlizzLikeString(LANG_OFFLINE), tele->name.c_str());
