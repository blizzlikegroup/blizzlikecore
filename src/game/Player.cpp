...
// helper function, mainly for script side, but can be used for simple task in BlizzLike also.
...
void Player::Regenerate(Powers power)
{
    ...
    switch (power)
    {
        case POWER_MANA:
        {
            ...
            if (recentCast)
            {
                // BlizzLike Updates Mana in intervals of 2s, which is correct
...
bool Player::resetTalents(bool no_cost)
{
    ...
    for (unsigned int i = 0; i < sTalentStore.GetNumRows(); ++i)
    {
        ...
        if ((getClassMask() & talentTabInfo->ClassMask) == 0)
            continue;
        // talent problem fix
        for (int prob = 37116; prob < 37118; ++prob)
            removeSpell(prob, false);
...
int32 Player::CalculateReputationGain(ReputationSource source, int32 rep, int32 faction, uint32 creatureOrQuestLevel, bool noAuraBonus)
{
    ...
    if (rate != 1.0f && creatureOrQuestLevel <= BlizzLike::XP::GetGrayLevel(getLevel()))
        percent *= rate;
...
bool Player::RewardHonor(Unit* uVictim, uint32 groupsize, float honor)
{
    ...
    if (honor <= 0)
    {
        ...
        if (uVictim->GetTypeId() == TYPEID_PLAYER)
        {
            ...
            k_grey = BlizzLike::XP::GetGrayLevel(k_level);
    ...
    for (GossipMenuItemsMap::const_iterator itr = pMenuItemBounds.first; itr != pMenuItemBounds.second; ++itr)
    {
        ...
        if (hasMenuItem)
        {
            ...
            if (isGMSkipConditionCheck)
            {
                ...
                strOptionText.append(GetSession()->GetBlizzLikeString(LANG_GM_ON));
                strOptionText.append(")");
            }
...
void Player::AddQuest(Quest const* pQuest, Object* questGiver)
{
    ...
    BLIZZLIKE_ASSERT(log_slot < MAX_QUEST_LOG_SIZE);
    ...
    for (Quest::PrevQuests::const_iterator iter = qInfo->prevQuests.begin(); iter != qInfo->prevQuests.end(); ++iter)
    {
        ...
        if (qPrevInfo && i_prevstatus != mQuestStatus.end())
        {
            ...
            if (*iter > 0 && i_prevstatus->second.m_rewarded)
            {
                ...
                BLIZZLIKE_ASSERT(bounds.first != bounds.second); // always must be found if qPrevInfo->ExclusiveGroup != 0
...
bool Player::SatisfyQuestExclusiveGroup(Quest const* qInfo, bool msg) const
{
    ...
    BLIZZLIKE_ASSERT(bounds.first != bounds.second);           // must always be found if qInfo->ExclusiveGroup != 0
...
{
    BLIZZLIKE_ASSERT(count < 256 && "mob/GO count store in 8 bits 2^8 = 256 (0..256)");
...
bool Player::LoadFromDB(ObjectGuid guid, SqlQueryHolder* holder)
{
    ...
    if (uint32 node_id = m_taxi.GetTaxiSource())
    {
        ...
        BLIZZLIKE_ASSERT(nodeEntry);                           // checked in m_taxi.LoadTaxiDestinationsFromString
...
void Player::HandleStealthedUnitsDetection()
{
    ...
    BlizzLike::AnyStealthedCheck u_check(this);
...
void Player::SendTransferAbortedByLockStatus(MapEntry const* mapEntry, AreaLockStatus lockStatus, uint32 miscRequirement)
{
    ...
    switch (lockStatus)
    {
        case AREA_LOCKSTATUS_TOO_LOW_LEVEL:
            GetSession()->SendAreaTriggerMessage(GetSession()->GetBlizzLikeString(LANG_LEVEL_MINREQUIRED), miscRequirement);
            break;
        ...
        case AREA_LOCKSTATUS_QUEST_NOT_COMPLETED:
            ...
            {
                GetSession()->SendAreaTriggerMessage(GetSession()->GetBlizzLikeString(LANG_TELEREQ_QUEST_BLACK_MORASS));
                break;
            }
...
void Player::RewardSinglePlayerAtKill(Unit* pVictim)
{
    ...
    uint32 xp = PvP ? 0 : BlizzLike::XP::Gain(this, pVictim);
...
{
    BLIZZLIKE_ASSERT((!GetGroup() || pRewardSource) && "Player::RewardPlayerAndGroupAtEvent called for Group-Case but no source for range searching provided");
