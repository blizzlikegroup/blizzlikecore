...
void SpellCastTargets::read(ByteBuffer& data, Unit* caster)
{
    ...
    if (m_targetMask & TARGET_FLAG_SOURCE_LOCATION)
    {
        ...
        if (!BlizzLike::IsValidMapCoord(m_srcX, m_srcY, m_srcZ))
    ...
    if (m_targetMask & TARGET_FLAG_DEST_LOCATION)
    {
        ...
        if (!BlizzLike::IsValidMapCoord(m_destX, m_destY, m_destZ))
            throw ByteBufferException(false, data.rpos(), 0, data.size());
    }
...
{
    BLIZZLIKE_ASSERT(caster != NULL && info != NULL);
    BLIZZLIKE_ASSERT(info == sSpellStore.LookupEntry(info->Id) && "`info` must be pointer to sSpellStore element");
...
WorldObject* Spell::FindCorpseUsing()
{
    ...
    BlizzLike::WorldObjectSearcher<T> searcher(result, u_check);
...
void Spell::SetTargetMap(SpellEffectIndex effIndex, uint32 targetMode, UnitList& targetUnitMap)
{
    ...
    switch (targetMode)
    {
        ...
        case TARGET_RANDOM_CIRCUMFERENCE_POINT:
        {
            ...
            // and no unitTarget (e.g. summon effects). As BlizzLike always needs a unitTarget we add just the caster here.
        ...
        case TARGET_RANDOM_ENEMY_CHAIN_IN_AREA:
        {
            ...
            {
                BlizzLike::AnyAoETargetUnitInObjectRangeCheck u_check(m_caster, max_range);
                BlizzLike::UnitListSearcher<BlizzLike::AnyAoETargetUnitInObjectRangeCheck> searcher(tempTargetUnitMap, u_check);
        ...
        case TARGET_RANDOM_FRIEND_CHAIN_IN_AREA:
        {
            ...
            UnitList tempTargetUnitMap;
            {
                BlizzLike::AnyFriendlyUnitInObjectRangeCheck u_check(m_caster, max_range);
                BlizzLike::UnitListSearcher<BlizzLike::AnyFriendlyUnitInObjectRangeCheck> searcher(tempTargetUnitMap, u_check);
                Cell::VisitAllObjects(m_caster, searcher, max_range);
            }
        ...
        case TARGET_CHAIN_DAMAGE:
        {
            ...
            else
            {
                ...
                UnitList tempTargetUnitMap;
                {
                    BlizzLike::AnyAoEVisibleTargetUnitInObjectRangeCheck u_check(pUnitTarget, originalCaster, max_range);
                    BlizzLike::UnitListSearcher<BlizzLike::AnyAoEVisibleTargetUnitInObjectRangeCheck> searcher(tempTargetUnitMap, u_check);
                    Cell::VisitAllObjects(m_caster, searcher, max_range);
                }
        ...
        case TARGET_AREAEFFECT_GO_AROUND_DEST:
        {
            ...
            for (SQLMultiStorage::SQLMultiSIterator<SpellTargetEntry> i_spellST = bounds.first; i_spellST != bounds.second; ++i_spellST)
            {
                ...
                if (i_spellST->type == SPELL_TARGET_TYPE_GAMEOBJECT)
                {
                    ...
                    BlizzLike::GameObjectEntryInPosRangeCheck go_check(*m_caster, i_spellST->targetEntry, x, y, z, radius);
                    BlizzLike::GameObjectListSearcher<BlizzLike::GameObjectEntryInPosRangeCheck> checker(tempTargetGOList, go_check);
            ...
            switch (m_spellInfo->Effect[effIndex])
            {
                case SPELL_EFFECT_DUMMY:
                {
                    switch (m_spellInfo->Id)
                    {
                        ...
                        {
                            WorldObject* result = FindCorpseUsing<BlizzLike::CannibalizeObjectCheck> ();
                ...
                for (SQLMultiStorage::SQLMultiSIterator<SpellTargetEntry> i_spellST = bounds.first; i_spellST != bounds.second; ++i_spellST)
                {
                    ...
                    switch (i_spellST->type)
                    {
                        case SPELL_TARGET_TYPE_GAMEOBJECT:
                        {
                            ...
                            {
                                BlizzLike::NearestGameObjectEntryInObjectRangeCheck go_check(*m_caster, i_spellST->targetEntry, range);
                        ...
                        default:
                        {
                            ...
                            if (!targetExplicit)
                            {
                                BlizzLike::NearestCreatureEntryWithLiveStateInObjectRangeCheck u_check(*m_caster, i_spellST->targetEntry, i_spellST->type != SPELL_TARGET_TYPE_DEAD, i_spellST->type == SPELL_TARGET_TYPE_DEAD, range);
...
{
    BlizzLike::SpellNotifierCreatureAndPlayer notifier(*this, targetUnitMap, radius, pushType, spellTargets, originalCaster);
