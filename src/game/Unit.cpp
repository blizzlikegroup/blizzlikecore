...
Unit::~Unit()
{
    ...
    BLIZZLIKE_ASSERT(m_gameObj.size() == 0);
    BLIZZLIKE_ASSERT(m_dynObjGUIDs.size() == 0);
    BLIZZLIKE_ASSERT(m_deletedAuras.size() == 0);
    BLIZZLIKE_ASSERT(m_deletedHolders.size() == 0);
}
...
uint32 Unit::DealDamage(Unit* pVictim, uint32 damage, CleanDamage const* cleanDamage, DamageEffectType damagetype, SpellSchoolMask damageSchoolMask, SpellEntry const* spellProto, bool durabilityLoss)
{
    ...
    else                                                    // if (health <= damage)
    {
        ...
        if (duel_hasEnded)
        {
            BLIZZLIKE_ASSERT(pVictim->GetTypeId() == TYPEID_PLAYER);
            Player* he = (Player*)pVictim;

            BLIZZLIKE_ASSERT(he->duel);
...
{
    BLIZZLIKE_ASSERT(pSpell);                                  // NULL may be never passed here, use InterruptSpell or InterruptNonMeleeSpells
...
{
    BLIZZLIKE_ASSERT(spellType < CURRENT_MAX_SPELL);
...
{
    BLIZZLIKE_ASSERT(gameObj && !gameObj->GetOwnerGuid());
...
{
    BLIZZLIKE_ASSERT(gameObj && gameObj->GetOwnerGuid().IsEmpty());
...
{
    BLIZZLIKE_ASSERT(gameObj && gameObj->GetOwnerGuid() == GetObjectGuid());
...
{
    BLIZZLIKE_ASSERT(GetTypeId() == TYPEID_UNIT);
...
{
    BLIZZLIKE_ASSERT(pTarget && GetTypeId() == TYPEID_UNIT);
...
Unit* Unit::SelectRandomUnfriendlyTarget(Unit* except /*= NULL*/, float radius /*= ATTACK_DISTANCE*/) const
{
    ...
    BlizzLike::AnyUnfriendlyUnitInObjectRangeCheck u_check(this, radius);
...
Unit* Unit::SelectRandomFriendlyTarget(Unit* except /*= NULL*/, float radius /*= ATTACK_DISTANCE*/) const
{
    ...
    BlizzLike::AnyFriendlyUnitInObjectRangeCheck u_check(this, radius);
...
class RelocationNotifyEvent : public BasicEvent
{
    public:
        ...
        bool Execute(uint64 /*e_time*/, uint32 /*p_time*/)
        {
            ...
            {
                BlizzLike::PlayerRelocationNotifier notify((Player&)m_owner);
                Cell::VisitAllObjects(&m_owner, notify, radius);
            }
            else // if(m_owner.GetTypeId() == TYPEID_UNIT)
            {
                BlizzLike::CreatureRelocationNotifier notify((Creature&)m_owner);
                Cell::VisitAllObjects(&m_owner, notify, radius);
            }
