...
{
    // Load EventAI Text
    sObjectMgr.LoadBlizzLikeStrings(WorldDatabase, "creature_ai_texts", MIN_CREATURE_AI_TEXT_STRING_ID, MAX_CREATURE_AI_TEXT_STRING_ID, true);
                    ...
                    case ACTION_T_TEXT:
                    {
                        ...
                        for (int k = firstTextParam; k < 3; ++k)
                        {
                            if (action.text.TextId[k])
                            {
                                ...
                                else if (!sObjectMgr.GetBlizzLikeStringLocale(action.text.TextId[k]))
                                {
