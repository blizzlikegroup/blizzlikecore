...
SqlPreparedStatement* SqlConnection::GetStmt(uint32 nIndex)
{
    ...
    if (m_holder[nIndex] == NULL)
    {
        ...
        BLIZZLIKE_ASSERT(fmt.length());
        // allocate SQlPreparedStatement object
        pStmt = CreateStatement(fmt);
        // prepare statement
        if (!pStmt->prepare())
        {
            BLIZZLIKE_ASSERT(false && "Unable to prepare SQL statement");
            return NULL;
        }
...
bool Database::CheckRequiredField(char const* table_name, char const* required_name)
{
    ...
    if (result2)
    {
        ...
        if (!reqName.empty())
        {
            ...
            sLog.outErrorDb();
            sLog.outErrorDb("  You have: --> `%s.sql`",cur_sql_update_name.c_str());
            sLog.outErrorDb();
            sLog.outErrorDb("  You need: --> `%s.sql`",req_sql_update_name);
            sLog.outErrorDb();
            sLog.outErrorDb("You must apply DB updates to use BlizzLikeCore.");
            sLog.outErrorDb("These updates are included in the (blizzlikecore/sql) folder.");
        }
        else
        {
            sLog.outErrorDb("The table `%s` in your [%s] database is missing its version info.",table_name,db_name);
            sLog.outErrorDb("BlizzLikeCore cannot find the version info needed to check that the db is up to date.");
            sLog.outErrorDb();
            sLog.outErrorDb("This revision of BlizzLikeCore requires a database updated to:");
...
{
    BLIZZLIKE_ASSERT(!m_pTrans);   // if we will get a nested transaction request - we MUST fix code!!!
